...
lora_config = LoraConfig(
    r=32,   #Количество дополнительных параметров, чем этот параметр выше, тем точнее               
    lora_alpha=64,   #коэффицент масштабирования, желательно, чтобы было в 2 раз больше чем количество доп. параметров        
...
training_args = TrainingArguments(
    ...
    num_train_epochs=12, #Модель 5 раз пройдёт по всем данным, чтобы повысить качество
    ...
    learning_rate=1e-4, #стандартная скорость обучения
    warmup_steps=50, 
    warmup_ratio=0.15, #штрафует за большие веса
    logging_steps=10, 
    save_steps=200, 
    save_total_limit=3, 
    ...
    report_to="none", 
    max_grad_norm=0.3, #Обрезает градиенты
)
#Запуск обучения
trainer = Trainer(
    model=model,
